...
--属性列表对话框
fd_dlg=LuaMaterialDialog(activity)
...
--子视图列表对话框
cd_dlg=LuaMaterialDialog(activity)
...
--可选属性对话框
check_dlg=LuaMaterialDialog(activity)
...
wds={
  ...
  {"LinearLayout","LinearLayoutCompat","AbsoluteLayout","FrameLayout","RelativeLayout","CoordinatorLayout","ConstraintLayout"},
  {"CardView","RadioGroup","GridLayout",
    "ScrollView","HorizontalScrollView","NestedScrollView"},
  {"AppBarLayout","CollapsingToolbarLayout","DrawerLayout","SwipeRefreshLayout","SlidingPaneLayout",
    "BottomAppBar","BottomNavigationItemView","BottomNavigationView",
    "MaterialButton","MaterialTextView","SwitchMaterial","MaterialSwitch","MaterialCardView","CircularRevealCardView","Chip","ChipGroup","CircularProgressIndicator","LinearProgressIndicator","CircularRevealFrameLayout",
    "CircularRevealGridLayout","FloatingActionButton","ExtendedFloatingActionButton","NavigationView","TabLayout","TextInputEditText","TextInputLayout"},
}
...
function onMenuItemSelected(id,item)
  ...
  if t=="复制" then
   ...
   elseif t=="保存" then
    ...
    local intent=Intent()
    intent.putExtra("alypath", luapath)
    activity.setResult(10000,intent)
    activity.finish()
   elseif item.getItemId()==android.R.id.home
    activity.finish()
  end
end
