...
local CustomTarget = bindClass("com.bumptech.glide.request.target.CustomTarget")
  ...
  if k=="layout_x" then
   ...
   elseif k=="layout_behavior" then
    ...
    if tostring(v) == "@string/appbar_scrolling_view_behavior" then
      ...
      local ScrollingViewBehavior = bindClass "com.google.android.material.appbar.AppBarLayout$ScrollingViewBehavior"
   ...
   elseif k=="background" then
    if type(v)=="string" then
      if v:find("^%?") then
        ...
        Glide.with(context).asBitmap().load(v)
        .into(CustomTarget{
